...
class Wavefunction(object):
    ...
    def mk(size, weights):
        ...
        """
        coefficients = Wavefunction.init_coefficients(size, list(weights.keys()))
    ...
    def collapse(self, co_ords):
        ...
        valid_weights = {tile: weight for tile, weight in iter(self.weights.items()) if tile in opts}

        total_weights = sum(valid_weights.values())
        rnd = rng.rand() * total_weights
...
model = Model((30, 75), weights, compatibility_oracle)
output = model.run()
